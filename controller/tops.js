...
router.dayproducts = (req, res, next) => {
    ...
    let TopProduct = [];
    let CategoriesName =[];
    con.query('select (p.name) as ProductName, count(p.id) as Count,(c.name) as categorieName from products p, order_detail od, orders o,categories c where p.id = od.product_id and o.id = od.order_id and c.id=p.category_id and  Date(created_at) ="'+fromDay+'" GROUP BY(od.product_id)ORDER BY Count DESC LIMIT 0,10', function (err, rows, fields) {
        if (err) throw err
